        ...
        if self.state == "find":
            ...
            if self.searched_marker is None:
                if (len(self.markers_list) == 0):
                    ...
                    self.get_logger().info(f"Searching marker {self.searched_marker}")
            else:
                if marker_ids is not None:
                    for i in range(len(marker_ids)):
                        if self.searched_marker == marker_ids[i]:
                            ...
                            if abs(marker_center[0] - image_center[0]) < 10 and abs(marker_center[1] - image_center[1]) < 10:
                                ...
                                cv2.putText(outputImage, f"id={int(marker_ids[i])}", (int(marker_center[0]), int(marker_center[1]) - marker_radius - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)

                                self.markers_pub.publish(self.bridge.cv2_to_imgmsg(outputImage, "bgr8"))
